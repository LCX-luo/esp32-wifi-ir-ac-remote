...
- **Debug / 调试阶段**：每次代码或文档修改后，立即 `git add` + `git commit` **本地同步**（CHANGELOG / error-fixes 同步更新），**不推送远端**。
- **功能验证通过后**：烧录/测试验证通过后，才 `git push` 推送远端（固件 → `main`，网页 → `gh-pages`）。
- **原则**：远端始终只保留"可用版本"，调试过程保留在本地 git 历史中。
