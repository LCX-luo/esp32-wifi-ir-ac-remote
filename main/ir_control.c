 ...
 * 编码规则（实测）：
 *   - A = 0xB2 固定用户码
 *   - B = (风速码<<5) | 0x1F：Auto=0xBF, Low=0x9F, Med=0x5F, High=0x3F
 *   - C = 温度×8（17~30°C，16°C 为特殊档 0x00）
 *   - 关机：B=0x7B, C=0xE0（特殊命令）
 *   - 扫风：独立命令 A=0xB9, B=0xF5, C=0x04
 */
#include <stdio.h>
#include "esp_log.h"
...
/* 手机实测硬编码信号 */
#define MIDEA_SWING_A      0xB9
#define MIDEA_SWING_B      0xF5
#define MIDEA_SWING_C      0x04
#define MIDEA_QUICKCOOL_B  0x3F   /* 16°C + 高风速 */
#define MIDEA_QUICKCOOL_C  0x00
...
/* 由 A/B/C 构造 48bit 并发射两帧 */
static void ir_send_abc(uint8_t A, uint8_t B, uint8_t C)
{
    uint64_t data = ((uint64_t)A << 40) | ((uint64_t)(uint8_t)~A << 32)
    ...
    n += midea_build_frame(data, syms + n);   /* 两帧相同 */
...
{
    (void)mode;   /* 模式编码未完全确认，暂固定制冷 */

    uint8_t A = MIDEA_USER_CODE, B, C;
    if (!power) {
        B = MIDEA_POWER_OFF_B;
        C = MIDEA_POWER_OFF_C;
    } else {
        B = (uint8_t)((s_fan_code[fan & 3] << 5) | 0x1F);
        C = (uint8_t)(temp * 8);   /* 17~30°C：C = 温度×8 */
    }
    ir_send_abc(A, B, C);
    ESP_LOGI(TAG, "IR send: power=%d fan=%d temp=%d (A=0x%02X B=0x%02X C=0x%02X)",
             power, fan, temp, A, B, C);
}

void ir_control_swing(void)
{
    ir_send_abc(MIDEA_SWING_A, MIDEA_SWING_B, MIDEA_SWING_C);
    ESP_LOGI(TAG, "IR swing toggle sent (A=0x%02X B=0x%02X C=0x%02X)",
             MIDEA_SWING_A, MIDEA_SWING_B, MIDEA_SWING_C);
}

void ir_control_quick_cool(void)
{
    /* 制冷 16°C 高风速（手机实测） */
    ir_send_abc(MIDEA_USER_CODE, MIDEA_QUICKCOOL_B, MIDEA_QUICKCOOL_C);
    ESP_LOGI(TAG, "IR quick cool: 16C high-fan sent");
    /* 开启扫风 */
    ir_send_abc(MIDEA_SWING_A, MIDEA_SWING_B, MIDEA_SWING_C);
    ESP_LOGI(TAG, "IR quick cool: swing on sent");
}
