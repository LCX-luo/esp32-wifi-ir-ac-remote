...
### 变更
- 回退推测的模式编码，稳定版仅保留：**开关 / 温度（C=温度×8，17~30°C）/ 风速 / 扫风**。
- **快速设置**（硬编码手机实测信号）：
  - 快速制冷：16°C 高风速 + 扫风（`quick_cool`）
  - 一键关机（`off`）
- **扫风**：独立命令 `swing`（`A=0xB9 B=0xF5 C=0x04`，手机实测）。
- 网页遥控器：新增"快速设置"卡片（快速制冷/一键关机）与"扫风切换"按钮，移除模式选择。
- 协议说明：关机 `B=0x7B C=0xE0`；**16°C 为特殊档 `C=0x00`**，17~30°C 才适用 `C=温度×8`。
...
- **数据传输可行性测试**：发射「引导码 + 8bit 数据 `0xA5`」帧，接收端解码字节并与发射数据比对，打印 MATCH / MISMATCH。
- **5V DAT 适配**：GPIO21 配置为输入 + 内部下拉，降低开漏型接收头空闲 5V 电平；注释建议推挽型输出串接红色 LED 限压。
